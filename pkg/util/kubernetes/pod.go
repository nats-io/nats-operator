...
func WaitUntilPodCondition(ctx context.Context, kubeClient corev1.CoreV1Interface, pod *v1.Pod, fn watch.ConditionFunc) error {
	...
	lw := &cache.ListWatch{
		ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
			...
			return kubeClient.Pods(pod.Namespace).List(context.TODO(), options)
		},
		WatchFunc: func(options metav1.ListOptions) (watchapi.Interface, error) {
			options.FieldSelector = fs.String()
			return kubeClient.Pods(pod.Namespace).Watch(context.TODO(), options)
		},
	}
...
func WaitUntilDeploymentCondition(ctx context.Context, kubeClient kubernetes.Interface, namespace, name string, fn watch.ConditionFunc) error {
	...
	lw := &cache.ListWatch{
		ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
			options.FieldSelector = fs.String()
			return kubeClient.AppsV1().Deployments(namespace).List(context.TODO(), options)
		},
		WatchFunc: func(options metav1.ListOptions) (watchapi.Interface, error) {
			options.FieldSelector = fs.String()
			return kubeClient.AppsV1().Deployments(namespace).Watch(context.TODO(), options)
		},
	}
