...
func NewMonkeys(kubecli corev1client.CoreV1Interface) *Monkeys {
...
func (m *Monkeys) CrushPods(ctx context.Context, c *CrashConfig) {
	...
	for {
		...
		pods, err := m.kubecli.Pods(ns).List(ctx, metav1.ListOptions{LabelSelector: ls})
		if err != nil {
		...
		for tokill := range tokills {
			delOpt := metav1.NewDeleteOptions(0)
			err = m.kubecli.Pods(ns).Delete(ctx, tokill, *delOpt)
			if err != nil {
