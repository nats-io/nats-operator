...
// TLSPolicy defines the TLS policy of a NATS cluster
...
type StaticTLS struct {
	// Member contains secrets containing TLS certs used by each NATS member pod.
...
type MemberSecret struct {
	// PeerSecret is the secret containing TLS certs used by each NATS member pod
	// for the communication between NATS peers.
	PeerSecret string `json:"peerSecret,omitempty"`
	// ServerSecret is the secret containing TLS certs used by each NATS member pod
	// for the communication between NATS server and its clients.
